...
#define ROAM_NEIGHBOR_PROCESS_TIME_MAX        (150)       /* ???????????? ????ms*/
...
#define ROAM_ENV_BETTER_RSSI_DISTANSE   (30)       /* ??????ESS??????????????AP??????????AP??RSSI???? */
...
typedef struct
{
    ...
    oal_uint32                 ul_scan_start_timetamp;          /* ?????????????????? */
    oal_uint32                 ul_scan_end_timetamp;            /* ?????????????????? */
    oal_uint32                 ul_connect_start_timetamp;       /* ?????????????????? */
    oal_uint32                 ul_connect_end_timetamp;         /* ?????????????????? */
