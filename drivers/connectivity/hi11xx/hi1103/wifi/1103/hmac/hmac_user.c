...
oal_uint8 hmac_user_calculate_num_spatial_stream(wlan_nss_enum_uint8  *puc_nss1, wlan_nss_enum_uint8 *puc_nss2)
{
    ...
    switch(*puc_nss1)
    {
        ...
        case 1:   /* 1??????????WLAN_SINGLE_NSS(????0) */
            *puc_nss1 = WLAN_SINGLE_NSS;
            break;

        case 2:   /* 2??????????WLAN_DOUBLE_NSS(????1) */
...
oal_void hmac_user_set_num_spatial_stream_160M(mac_user_stru *pst_mac_user)
{
    ...
    wlan_nss_enum_uint8           en_user_num_spatial_stream1 = ((pst_mac_user->en_user_num_spatial_stream + 1) >> 1); /* ???????????????????????? 0 ?? 1 ?? 2 */
    wlan_nss_enum_uint8           en_user_num_spatial_stream2 = (((pst_mac_user->en_user_num_spatial_stream  + 1) * 3) >> 2);  /* ???????????????????????? 0 ?? 1 ?? 2 ?? 3  */
