                ...
                mac_twt_action_mgmt_args_stru  *pst_twt_action_args)
{
    ...
    pst_twt_element.st_request_type.bit_setup_command = pst_twt_action_args->uc_twt_setup_cmd;/* ??user???? */
    pst_twt_element.st_request_type.bit_trigger       = 1;
    pst_twt_element.st_request_type.bit_implicit      = 1;
    pst_twt_element.st_request_type.bit_flow_type     = pst_twt_action_args->uc_twt_flow_type; /*??user????, annouced or unannounced */
    pst_twt_element.st_request_type.bit_flow_id       =  pst_twt_action_args->uc_twt_flow_id; /* ??user????*/
    ...
    pst_twt_element.ull_twt = pst_twt_action_args->ul_twt_start_time_offset;/* To-Do,  user decide, unit:   0 or positive number(according to TSF, ??DMAC ???? */
                ...
                mac_twt_action_mgmt_args_stru  *pst_twt_action_args)
{
    ...
    pst_twt_teardown.bit_twt_flow_id = pst_twt_action_args->uc_twt_flow_id;/* ??user????*/
                ...
                mac_twt_action_mgmt_args_stru  *pst_twt_action_args)
{
    ...
    MAC_GET_CB_MPDU_LEN(pst_tx_ctl) = us_frame_len + OAL_SIZEOF(dmac_ctx_action_event_stru);;
    MAC_GET_CB_FRAME_TYPE(pst_tx_ctl) = WLAN_CB_FRAME_TYPE_ACTION;
    MAC_GET_CB_FRAME_SUBTYPE(pst_tx_ctl) = WLAN_ACTION_TWT_TEARDOWN_REQ;  /* ??dmac_tx_action_process  skip, ????send to air */
                ...
                oal_uint8                  *puc_payload)
{
    ...
    else
    {
        ...
        /* ????????????256 buff??????????user????????????????????????????????????pst_ba_rx_stru->us_baw_size */
