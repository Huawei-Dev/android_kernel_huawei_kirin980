...
/*****************************************************************************
  2.5.2 AMPDU????
*****************************************************************************/
#define WLAN_AMPDU_RX_HE_BUFFER_SIZE    256  /* AMPDU??????????????????buffer size?????? */
...
#define WLAN_AMPDU_DISORDER_AP_MAX_NUM  4  /* ????????????????????AP AMPDU????????????????MPDU???? */
...
#define WLAN_LOW_RATE_DEC_AGC_TH          (100)              /* ??100ms????????????????agc target?????? */
#define WLAN_LOW_RATE_DEC_AGC_COUNT       (3)              /* ??100ms????????????????agc target?????????? */
...
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1105_DEV)
/* bfee 8*2?????????? he mu bfee???????????? */
/* buffer size = 26*(7+9)/2*500/8(160M) = 13000bytes,??????snr????10bytes,
   HE MU exculsive beamforming report??????????Ns * ??Nc *4?? = 500 * 2 * 4 = 500bytes
   ????????????????14000bytes */
...
#define HE_BFEE_NTX_SUPP_STS_CAP_EIGHT                (8)     /* 1105????????????NDP Nsts???? */
#define HE_BFEE_NTX_SUPP_STS_CAP_FOUR                 (4)     /* 1103????????????NDP Nsts???? */
