...
/* ????802.11-2016 9.4.1.1 Authentication Algorithm Number field ???????????????????? */
...
typedef enum
{
    ...
    *************************************************************************/
    ...
    WLAN_CFGID_SNIFFER              = 106,               /* ???????? */
    WLAN_CFGID_MONITOR_MODE         = 107,               /* ????monitor??????????????????,????monitor???????????????? */
#endif
...
//    WLAN_CFGID_GET_SMPS_EN           = 472,
    ...
    WLAN_CFGID_CFG80211_EXTERNAL_AUTH   = 564,     /* ????SAE???? */
    ...
    WLAN_CFGID_SET_OWE                  = 795,           /* ????APUT????????????OWE group */

    WLAN_CFGID_SET_MAC_FREQ             = 798,         /* MAC???????????????????? */

#ifdef _PRE_WLAN_FEATURE_11AX
    WLAN_CFGID_11AX_DEBUG                   = 799,
    WLAN_CFGID_TWT_SETUP_REQ                    = 800,      /* ????TWT?????????????? */
    WLAN_CFGID_TWT_TEARDOWN_REQ                    = 801,      /*  ????TWT?????????????? */
#endif

    WLAN_CFGID_SPEC_FEMLP_EN            = 802,  /* ????????fem??????????????*/
    WLAN_CFGID_SOFTAP_MIMO_MODE         = 803,

#ifdef _PRE_WLAN_FEATURE_HIMIT
    WLAN_CFGID_HIMIT_SWITCH             = 804,  /* ????himit???? */
#endif
...
#ifdef _PRE_WLAN_FEATURE_IP_FILTER
        WLAN_CFGID_ASSIGNED_FILTER      = 806,          /* ??????????????????????*/
#endif //_PRE_WLAN_FEATURE_IP_FILTER

#ifdef _PRE_WLAN_FEATURE_ALWAYS_TX
        WLAN_CFGID_SET_ALWAYS_TX_AGGR_NUM   = 807,          /* ???????????????? */
#endif

#if defined(_PRE_WLAN_FEATURE_11AX) && defined(_PRE_WLAN_FEATURE_UPH_AUTO)
    WLAN_CFGID_HE_UPH_CONTROL_ACTIVATED = 808,  /* ????dot11HEUPHControlActivated?? */
#endif
...
#ifdef _PRE_WLAN_FEATURE_STA_PM
    WLAN_CFGID_SET_FASTSLEEP_PARA          = 812,  /* ????fastsleep???????? */
#endif
    /************************************************************************
        ?????? ??MIB????????????????????????????
    *************************************************************************/
