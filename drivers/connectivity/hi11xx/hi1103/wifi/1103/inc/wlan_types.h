...
typedef enum
{
    ...
    WLAN_SCAN_FLAG_LOW_POWER     = BIT9, /*????????*/
    WLAN_SCAN_FLAG_HIFH_ACCURACY = BIT10,/*????????,?????? */
    WLAN_SCAN_FLAG_BUTT
} wlan_scan_flag_enum;
...
/* ??????, ????GI???????????????? HAL_GET_DSCR_GI_TYPE ?? HAL_GET_ALG_GI_TYPE????????*/
typedef enum
{
...
typedef struct
{
    oal_int16   s_per20m_idle_pwr[8];         //????20M????????????????(????p2m_per_20m_cca_st[0] ~ p2m_per_20m_cca_st[7])
    oal_uint32  aul_free_chn_per20m_time[8];  //????20M????????????????????(????p2m_per_20m_cca_st[0] ~ p2m_per_20m_cca_st[7])
