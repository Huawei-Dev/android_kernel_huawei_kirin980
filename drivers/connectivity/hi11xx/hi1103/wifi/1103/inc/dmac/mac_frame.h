...
#define MAC_XCAPS_EX_TWT_LEN        10    /* ????????5??????11ac Operating Mode Notification??????????bit62??????????8,ftm??????????????9, twt??????????????10 */
#endif
...
#define MAC_WLAN_OUI_TYPE_HAUWEI_COWORK     0x20        /* ????????Adaptive 11r*/
...
typedef enum
{
    ...
    MAC_EID_MULTI_BSSID_INDEX      = 85,   /*  m-bssid ????*/
...
typedef struct mac_frame_he_phy_cap mac_frame_he_phy_cap_stru; /*?????????? phy cap ????*/
...
struct mac_ext_cap_twt_ie
{
    ...
    oal_uint8   bit_resv16                : 5,
                bit_twt_requester_support   : 1,  /* 11ax TWT requester ???????? */
                bit_resv17                        : 2;
}__OAL_DECLARE_PACKED;
