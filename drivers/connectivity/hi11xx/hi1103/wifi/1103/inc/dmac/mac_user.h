...
typedef struct
{
    ...
    oal_uint8                          bit_change_announce_bss_color       : 6,         /*????bss color change announcement ie????*/
...
typedef struct
{
    ...
    wlan_nss_enum_uint8   en_user_num_spatial_stream_160M;        /* ??????????160M?????????????????? */
...
/* vht cap ie Extended nss bw support ???????? */
typedef enum
{
...
typedef struct
{
    ...
    oal_uint8                               uc_resource_req_buffer_threshold_exp;   /* ???????????????????? */
    oal_uint8                               uc_resv1;
...
#if defined(_PRE_WLAN_FEATURE_11AX) || defined(_PRE_WLAN_FEATURE_11AX_ROM)
    oal_bool_enum_uint8                     en_he_hdl_rom_no_use;                            /*rom ??????????????*/
    oal_uint8                               auc_he_hdl_rom_rsv1[28];
    oal_uint8                               bit_he_hdl_rom_rsv2                        : 5,
                                            bit_he_hdl_rom_vht_present_no_use          : 1,  /*rom ??????????????*/
                                            bit_he_hdl_rom_rsv3                        : 2;
    oal_uint8                               auc_he_hdl_rom_rsv4[4];
    oal_uint8                               uc_he_hdl_rom_bw_no_use;                         /*rom ??????????????*/
    oal_uint8                               uc_he_hdl_rom_center_freq_seg0_no_use;           /*rom ??????????????*/
