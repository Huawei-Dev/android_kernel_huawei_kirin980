...
typedef enum
{
    MAC_RX_ICMP_FILTER_STOPED  = 0, //??????????????????????????????????????????????
    MAC_RX_ICMP_FILTER_WORKING = 1, //??????????????????????????
...
{
    mac_assigned_filter_id_enum_uint8       en_filter_id;
    mac_icmp_filter_state_enum_uint8        en_state;        //????????????????????????
    oal_uint8                               uc_resv[2];
...
typedef struct
{
    ...
    oal_bool_enum_uint8                 uc_neighbor_report_process_flag;    /* ??????????????????neighbor report???? */
...
typedef struct
{
    ...
    oal_uint32                          ul_scan_flag;           /*??????????????????,????bit??????wlan_scan_flag_enum??????????????????????????*/
...
typedef struct
{
    ...
    oal_bool_enum_uint8     en_sp_reuse_debug_switch;    /* ??????????????spatial reuse?????????????? */
    oal_uint8               auc_resv[2];
...
#endif
    oal_uint64                          ul_wpa_rpt_time;                    /* ????????????supplicant???? */
...
typedef struct
{
    ...
    oal_uint8                               en_11ax_switch            : 1,          /*11ax????*/
...
typedef struct
{
    oal_uint32                          ul_short_sleep_cnt;   /* idle???? 60ms ???? */
    oal_uint32                          ul_long_sleep_cnt;    /* idle???? 200ms ???? */
    oal_bool_enum_uint8                 en_query_completed_flag;
...
typedef struct
{
    ...
    oal_uint8   uc_wifi_bw;                 /* WiFi???? */
    oal_uint8   uc_modem_bw;                /* Modem???? */
    oal_uint8   uc_wifi_avoid_flag;         /* WiFi???????????? */
    oal_uint8   uc_wifi_priority;           /* WiFi?????? */
