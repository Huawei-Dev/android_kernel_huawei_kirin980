...
#define DMAC_WLAN_NRCOEX_INTERFERE_RULE_NUM     (4) /* 5gnr??????????????????????4?? */
#endif
...
typedef enum
{
    ...
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_TWT_SETUP,  /* STA????twt ?????????????? */
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_STA_SET_NDP_FEEDBACK_REPORT_REG,   /* STA????beacon??assoc rsp????????NDP Feedback report?????? */
#endif
...
struct  mac_tx_ctl
{
    ...
    /* byte19 */
    oal_uint8                               bit_large_amsdu_level       :2;          /* offload??????AMSDU?????? */
    oal_uint8                               bit_align_padding           :2;          /* SKB ????????ETHER HEADER??,4??????????????padding */
    oal_uint8                               bit_msdu_num_in_amsdu       :2;          /* ??????????????AMSDU?????? */
    oal_uint8                               bit_is_large_skb_amsdu      :1;          /* ???????????????????? */
    oal_uint8                               bit_htc_flag                :1;          /* ????????htc */
...
typedef struct
{
    ...
    oal_uint8                       uc_hw_ampdu_first_flag;    /* ???????????????????????? */
    oal_uint8                       auc_resv[3];
#endif
}dmac_tid_rom_stru;
...
typedef struct
{
    ...
    oal_uint32              ul_tid_buffer_frame_length;        /* ??tid???????????? */
    ...
    wlan_tx_amsdu_enum_uint8  en_last_amsdu_level;       /* ????????????amsdu */
