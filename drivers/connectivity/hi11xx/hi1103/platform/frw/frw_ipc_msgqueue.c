...
/*
 * ?? ?? ??  : frw_ipc_msg_queue_send_etc(ipc_node *node, oal_netbuf_t *buf);
 * ????????  : ??????????????????
...
/*
 * ?? ?? ??  : frw_ipc_msg_queue_register_callback_etc
 * ????????  : ????????????
 * ????????  : p_ipc_msg_handler: ??????????????????
 */
