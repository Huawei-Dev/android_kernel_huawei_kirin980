...
typedef enum {
    ...
    OAL_IEEE80211_MLME_ASSOC_RSP = 1,   /* MLME?????????????????? */
    OAL_IEEE80211_MLME_REASSOC_RSP = 2, /* MLME???????????????????? */
...
typedef struct oal_vap_status_diag_info {
    ...
    oal_uint8 auc_hisi_cca_shreshold[OAL_VAP_CCA_LEN];  // ??????????   3.10.1
...
typedef struct oal_sta_status_diag_info {
    ...
    oal_uint8 auc_chip_vendor[OAL_STA_CHIP_VENDOR_LEN]; /* ????????????: */
    ...
    oal_uint32 aul_sta_tx_mcs_cnt[OAL_STA_TX_MCS_LEN]; /* how many tx mcs count ????????????????????MCS??????,????0 */
    oal_uint32 aul_sta_rx_mcs_cnt[OAL_STA_RX_MCS_LEN]; /* how many rx mcs count ????????????????????MCS??????,????0 */
