...
/* 
 * ??????????????????????: WLAN_ASSOC_USER_MAX_NUM????g_us_assoc_max_user??
 * WLAN_ACTIVE_USER_MAX_NUM????g_us_active_max_user
 */
/* ?????????????????? */
...
/* ????512(????????????) + 64(????????????) */ /* ????! ??????????????????????oal_mem.c????OAL_MEM_BLK_TOTAL_CNT */
...
/* ????hmac_vap_cfg_priv_stru??????VAP???? + ???????? FRW_EVENT_TYPE_BUTT * WLAN_VAP_SUPPORT_MAX_NUM_LIMIT */
...
/* ??????((WLAN_VAP_SUPPORT_MAX_NUM_LIMIT - 1) * 2) */ /* ????VAP????MIB */
...
/* 1500 + WLAN_MAX_FRAME_HEADER_LEN(36) + WLAN_HDR_FCS_LENGTH(4) + (????????????,????????????????(20)) */
