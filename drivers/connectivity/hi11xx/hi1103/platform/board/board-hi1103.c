...
int32 hi1103_board_flowctrl_gpio_init_etc(void)
{
    ...
    g_board_info_etc.flowctrl_gpio = physical_gpio;
...
{
    oal_gpio_free(g_board_info_etc.flowctrl_gpio);
}
...
void hi1103_dump_gpio_regs(void)
{
    ...
    value = 0;
    ret = read_device_reg16(GPIO1_BASE_ADDR + GPIO_LEVEL_GET_REGADDR, &value);
    if(ret)
    {
        return;
    }
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "gpio reg 0x%x = 0x%x", GPIO1_BASE_ADDR + GPIO_LEVEL_GET_REGADDR, value);
...
int32 hi1103_check_wlan_flow_ctrl(void)
{
    int32 i;
    uint16 value;
    int32 ret;
    ...
    if (g_board_info_etc.flowctrl_gpio == 0) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "wlan_flow_ctrl gpio is zero!");
        return -OAL_EIO;
    }

    value = 0;
    ret = read_device_reg16(GPIO1_BASE_ADDR + GPIO_INOUT_CONFIG_REGADDR, &value);
    if (ret) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "read 0x%x reg failed, ret=%d",
                             (GPIO1_BASE_ADDR + GPIO_INOUT_CONFIG_REGADDR), ret);
        return -1;
    }
    ...
    ret = write_device_reg16(GPIO1_BASE_ADDR + GPIO_INOUT_CONFIG_REGADDR, value);
    if (ret) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "write 0x%x reg failed,value=0x%x, ret=%d",
                             (GPIO1_BASE_ADDR + GPIO_INOUT_CONFIG_REGADDR), value, ret);
        return -1;
    }

    for (i = 0; i < ul_test_times; i++) {
        ret = write_device_reg16(GPIO1_BASE_ADDR + GPIO_LEVEL_CONFIG_REGADDR, WLAN_FLOWCTRL_GPIO);
        if (ret) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "write 0x%x reg failed,value=0x%x, ret=%d",
                                 (GPIO1_BASE_ADDR + GPIO_LEVEL_CONFIG_REGADDR), WLAN_FLOWCTRL_GPIO, ret);
            return -1;
        }

        oal_msleep(1);

        if (0 == gpio_get_value(g_board_info_etc.flowctrl_gpio)) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "pull gpio high failed!");
            hi1103_dump_gpio_regs();
            return -OAL_EFAIL;
        }

        ret = write_device_reg16(GPIO1_BASE_ADDR + GPIO_LEVEL_CONFIG_REGADDR, 0x0);
        if (ret) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "write 0x%x reg failed,value=0x%x, ret=%d",
                                 (GPIO1_BASE_ADDR + GPIO_LEVEL_CONFIG_REGADDR), WLAN_FLOWCTRL_GPIO, ret);
            return -1;
...
int32 hi1103_dev_io_test(void)
{
    ...
    ret = hi1103_check_wlan_flow_ctrl();
    if (ret) {
        if (!ft_fail_powerdown_bypass) {
            hi1103_board_power_off(WLAN_POWER);
        }
