...
#define PRIV_CMD_STRU_HEAD_LEN      (8)   /* android private cmd:l_total_len与l_used_len所占空间长度 */
...
OAL_STATIC oal_uint32 wal_ioctl_judge_input_param_length(wal_android_wifi_priv_cmd_stru st_priv_cmd, oal_uint32 ul_cmd_length, oal_uint16 us_adjust_val)
{
    ...
    {
        /* 入参长度不满足要求, 申请的内存pc_command统一在主函数wal_android_priv_cmd中释放 */
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    /* 当ini中coutry_code=99时，不接受android修改country_code的命令，还是使用ini中country_code */
