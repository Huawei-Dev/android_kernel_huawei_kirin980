...
#define PRIV_CMD_STRU_HEAD_LEN      (8)   /* android private cmd:l_total_len??l_used_len???????????? */
...
OAL_STATIC oal_int32 wal_ioctl_set_vowifi_param(oal_net_device_stru *pst_net_dev, oal_int8* puc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd);
...
OAL_STATIC oal_int32 wal_ioctl_priv_cmd_set_ap_wps_p2p_ie( oal_net_device_stru * pst_net_dev, oal_int8 * pc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd);
...
#endif
    {"dscr_th_opt",              wal_hipriv_dscr_th_opt},       /* ????host??rx??????????????????: "wlan0 dscr_th_opt <queue id> <start_th> <interval>" */
    {"tcp_ack_filter",           wal_hipriv_tcp_ack_filter},    /* ????tcp ack????????????: "wlan0 tcp_ack_filter <0|1>(off/on) */
};
...
OAL_STATIC oal_uint32 wal_ioctl_judge_input_param_length(wal_android_wifi_priv_cmd_stru st_priv_cmd, oal_uint32 ul_cmd_length, oal_uint16 us_adjust_val)
{
    /* ????+1?? ?????????????????????????????????? */
    if (st_priv_cmd.ul_total_len < ul_cmd_length + 1 + us_adjust_val)
    {
        /* ??????????????????, ??????????pc_command????????????wal_android_priv_cmd?????? */
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC oal_int32 wal_ioctl_priv_cmd_tx_power(oal_net_device_stru * pst_net_dev, oal_int8 * pc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd)
{
...
#ifdef _PRE_WLAN_FEATURE_LTECOEX
OAL_STATIC oal_int32 wal_ioctl_priv_cmd_ltecoex_mode(oal_net_device_stru * pst_net_dev, oal_int8 * pc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd)
{
    oal_int32      l_ret;
...
OAL_STATIC oal_int32 wal_ioctl_priv_cmd_country(oal_net_device_stru * pst_net_dev, oal_int8 * pc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd)
{
...
oal_int32 wal_android_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, oal_int32 ul_cmd)
{
    ...
    /* ????????????wpa_supplicant ???????????????? */
    pc_command = oal_memalloc((oal_uint32)(st_priv_cmd.ul_total_len + 1));/* total len ??priv cmd ????buffer ???? */
    ...
    if (oal_strncmp(pc_command, CMD_SET_HITALK_STR, OAL_STRLEN(CMD_SET_HITALK_STR)) == 0)
    {
        ...
        {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_android_priv_cmd: set hitalk when pm open");
        }
        else
        {
            hitalk_status = NARROW_BAND_ON_MASK | NBFH_ON_MASK;
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "wal_android_priv_cmd: set hitalk enable");
        }
    }
    else
#endif
    ...
    else if(oal_strncmp(pc_command, CMD_P2P_SET_NOA, CMD_P2P_SET_NOA_LEN) == 0)
    {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_P2P_SET_NOA length is too short! at least need [%d]!}\r\n", skip + OAL_SIZEOF(st_p2p_noa_param));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
    ...
    else if(oal_strncmp(pc_command, CMD_P2P_SET_PS, CMD_P2P_SET_PS_LEN) == 0)
    {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_P2P_SET_PS length is too short! at least need [%d]!}\r\n", skip + OAL_SIZEOF(st_p2p_ops_param));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
   ...
   else if(0 == oal_strncmp(pc_command, CMD_SET_STA_PM_ON, CMD_SET_STA_PM_ON_LEN))
   {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_SET_STA_PM_ON length is too short! at least need [%d]!}\r\n", (CMD_SET_STA_PM_ON_LEN + 2));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
    ...
    else if (0 == oal_strncmp(pc_command, CMD_SET_QOS_MAP, CMD_SET_QOS_MAP_LEN))
    {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_SET_QOS_MAP length is too short! at least need [%d]!}\r\n", (skip + OAL_SIZEOF(st_qos_map_param)));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
    ...
    else if (0 == oal_strncmp(pc_command, CMD_SETSUSPENDMODE, CMD_SETSUSPENDMODE_LEN))
    {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_SETSUSPENDMODE length is too short! at least need [%d]!}\r\n", (CMD_SETSUSPENDMODE_LEN + 2));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
    ...
    else if (oal_strncmp(pc_command, CMD_FILTER_SWITCH, CMD_FILTER_SWITCH_LEN) == 0)
    {
        ...
        if (OAL_SUCC != l_ret)
        {
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_android_priv_cmd:: CMD_FILTER_SWITCH length is too short! at least need [%d]!}\r\n", (CMD_FILTER_SWITCH_LEN + 2));
            oal_free(pc_command);
            return -OAL_EFAIL;
        }
...
OAL_STATIC oal_int32 wal_ioctl_set_vowifi_param(oal_net_device_stru *pst_net_dev, oal_int8* puc_command, wal_android_wifi_priv_cmd_stru st_priv_cmd)
{
    ...
    for (ul_cfg_id = VOWIFI_SET_MODE; ul_cfg_id < VOWIFI_CMD_BUTT; ul_cfg_id++)
    {
        if ((0 == oal_strncmp(puc_command, g_ast_vowifi_cmd_table[ul_cfg_id].pc_priv_cmd, OAL_STRLEN(g_ast_vowifi_cmd_table[ul_cfg_id].pc_priv_cmd))) &&
            (st_priv_cmd.ul_total_len > (OAL_STRLEN(g_ast_vowifi_cmd_table[ul_cfg_id].pc_priv_cmd) + 1))) //1????????????
        {
