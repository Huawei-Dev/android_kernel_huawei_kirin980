...
#define PRIV_CMD_STRU_HEAD_LEN     (8)     /* private cmd:l_total_len??l_used_len???????????? */
...
OAL_STATIC OAL_CONST oal_iw_handler g_ast_iw_handlers[] =
{
    ...
    (oal_iw_handler)wal_ioctl_get_mode,         /* SIOCGIWMODE, ????bss type */
...
OAL_STATIC oal_int32  wal_netdev_stop_sta_p2p(oal_net_device_stru *pst_net_dev)
{
    ...
    if (((pst_net_dev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) || (pst_net_dev->ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_DEVICE))
         &&((oal_strcmp("wlan0", pst_net_dev->name) == 0) || (oal_strcmp("p2p0", pst_net_dev->name) == 0))) {
        /* ????????p2p????,??????p2p0??????p2p??????????p2p/sta??????????????mac??????wal_netdev_stop?????????????? */
...
OAL_STATIC oal_uint32 wal_ioctl_judge_input_param_length(wal_wifi_priv_cmd_stru st_priv_cmd, oal_uint32 ul_cmd_length, oal_uint16 us_adjust_val)
{
    ...
    {
        /* ??????????????????, ??????????pc_command????????????wal_vendor_priv_cmd?????? */
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    /* ??ini??coutry_code=99??????????????country_code????????????????ini??country_code */
...
oal_int32  wal_set_random_mac_to_mib(oal_net_device_stru *pst_net_dev)
{
    ...
    switch (pst_wdev->iftype)
    {
        ...
        case NL80211_IFTYPE_P2P_GO:
            ...
            /* ????????????????????P2P interface MAC ???? */
            /* ??????????????????mac????,??wal_cfg80211_add_p2p_interface_init?????? */
