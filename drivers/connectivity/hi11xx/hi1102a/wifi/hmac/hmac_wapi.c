...
{
    oal_uint32           ul_ret;
    oal_netbuf_stru     *pst_netbuf_tmp = OAL_PTR_NULL;    /* ??????????????netbuff */
    oal_netbuf_stru     *pst_netbuf_prev = OAL_PTR_NULL;   /* ??????????????netbuff */
    oal_netbuf_stru     *pst_buf_first = OAL_PTR_NULL;     /* ??????????????netbuff */
    //oal_uint16           us_len;
...
oal_uint32 hmac_wapi_decrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru  *pst_netbuf)
{
    ...
    oal_uint8                    *puc_mic_data = OAL_PTR_NULL;        /* ??????????????mic???????????????? wapi???????? ??51 */
