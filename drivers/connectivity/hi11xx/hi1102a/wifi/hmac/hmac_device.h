...
typedef struct {
    oal_uint32 ul_tx_large_pps;          /* ????????????tx pps */
    oal_uint32 ul_rx_large_pps;          /* ????????????rx pps */
    oal_uint32 ul_tx_small_pps;          /* ????????????tx pps */
    oal_uint32 ul_rx_small_pps;          /* ????????????rx pps */
    oal_uint32 ul_rx_large_amsdu_pps;    /* ????????????rx ????amsdu pps */
...
#endif
    oal_bool_enum_uint8     en_txop_limit;          /* ????????txop */
    hmac_dscr_th_opt_stru   st_dscr_th_opt;         /* ???????????????????? */
