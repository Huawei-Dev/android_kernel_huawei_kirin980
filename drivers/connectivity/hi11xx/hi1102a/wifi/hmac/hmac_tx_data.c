...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
        st_action_args.ul_arg2     = (oal_uint32)wlan_customize.ul_ampdu_tx_max_num;       /* ADDBA_REQ????buffer_size?????????? */
...
OAL_INLINE oal_uint32  hmac_tx_encap(
                ...
                oal_netbuf_stru  *pst_buf)
{
    mac_ieee80211_qos_htc_frame_addr4_stru   *pst_hdr = OAL_PTR_NULL;           /* 802.11?? */
    mac_ether_header_stru                    *pst_ether_hdr = OAL_PTR_NULL;
...
OAL_STATIC OAL_INLINE oal_uint32  hmac_tx_lan_mpdu_process_ap(
                ...
                mac_tx_ctl_stru    *pst_tx_ctl)
{
    hmac_user_stru          *pst_user = OAL_PTR_NULL;      /* ????STA?????? */
    mac_ether_header_stru   *pst_ether_hdr; /* ???????? */
    oal_uint8               *puc_addr;      /* ???????? */
    oal_uint32               ul_ret;
    oal_uint16               us_user_idx;
...
oal_uint32  hmac_tx_wlan_to_wlan_ap(oal_mem_stru *pst_event_mem)
{
    frw_event_stru         *pst_event = OAL_PTR_NULL;        /* ?????????? */
    mac_vap_stru           *pst_mac_vap = OAL_PTR_NULL;
    oal_netbuf_stru        *pst_buf = OAL_PTR_NULL;          /* ??netbuf????????????????netbuf?????? */
