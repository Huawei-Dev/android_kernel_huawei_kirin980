...
typedef struct
{
    ...
    oal_bool_enum_uint8                     en_tcp_ack_filtered[2];     /* bitmap, ??MPDU??????tcp ack???????????? */
} hmac_rx_buf_stru;
...
typedef struct
{
    ...
    oal_uint8                            uc_reject_bsstreq_times;                /* ??????????????AP?????????? */
    oal_bool_enum_uint8                  en_bsstreq_filter;                      /* ??????????????dmac????11v bsst req?? */
    oal_uint8                            auc_reserve[2];
    mac_user_callback_func_11v           mac_11v_callback_fn;                    /* ???????????? */
    oal_uint8                            auc_target_bss_addr[WLAN_MAC_ADDR_LEN]; /* ??????????????MAC???? */
