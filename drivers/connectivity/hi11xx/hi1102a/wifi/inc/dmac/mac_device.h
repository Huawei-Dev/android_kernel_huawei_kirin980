...
#define     MAC_FCS_DEFAULT_PROTECT_TIME_OUT5   9000    /* us ??VAP??????????????????null????????????????null?????????????? */ 
...
typedef struct
{
    ...
    union{
        ...
        oal_uint32            ul_work_time_ms;                        /* ??????????????ROM??,??????work_time */
    };
    union{
        oal_int32             l_rssi_thold;                           /* ????????rssi???? */
        oal_uint32            ul_listen_interval_ms;                  /* ??????????????ROM??,?????????? */
    };
...
typedef enum
{
    ...
    MAC_SCAN_REFUSED = 3,       /* ?????????? */
    MAC_SCAN_ABORT   = 4,       /* ???????? */
    MAC_SCAN_ABORT_SYNC = 5,    /* ????????????????????????????????????????????abort??????????????????scan abort????????dmac????abort?????????????? */
...
typedef struct
{
    ...
    oal_bool_enum_uint8                 en_11k_capable;                     /* ????????11k */
    oal_bool_enum_uint8                 en_11v_capable;                     /* ????????11v */
...
/* end: P2P */
    ...
    oal_bool_enum_uint8                 en_fft_window_offset_enable;           /* ????????????FFT???? */
