 ...
 * ????????  : bfgx open??????????????????????exception info
 * ????????  : subsys_type:??????????????????????WIFI????BFGX
 *             thread_type:????????????????
 *             exception_type:??????????
 * ????????  : ??
 * ?? ?? ??  : ????????????????0??????????1
...
/*
 * ?? ?? ??  : wifi_system_reset
 * ????????  : wifi??????????????device??????????bfgn??wifi firmware??????????
 *              ????????????????bfgn??????
 * ????????  : ??
 * ????????  : ??
 * ?? ?? ??  : ????????????????0??????????1
 ...
 * ????????  : ????????????????????wifi????????
 * ????????  : st_exception_info *exception????????????????????????????????wifi
 *             ????BFGN??????BFGN????????????????????????????????????????
 * ????????  : ??
 * ?? ?? ??  : ????????????????0??????????1
 */
...
/*
 * ?? ?? ??  : bfgx_system_reset
 * ????????  : bfgin??????????????????device??????????wifi??bfgin firmware????
 *             ??????????wifi??????bfgin??????
 * ????????  : ??
 * ????????  : ??
 * ?? ?? ??  : ????????????????0??????????1
...
int32 bfgx_store_stack_mem_to_file(void)
{
    ...
    for (i = 0; i < BFGX_MEM_DUMP_BLOCK_COUNT; i++) {
        ...
        snprintf_s(filename, sizeof(filename), sizeof(filename) - 1, BFGX_DUMP_PATH "/%s_%s.bin",
...
int32 sdio_get_and_save_data(exception_bcpu_dump_msg *sdio_read_info, uint32 count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        while (index < send_total_len) {
            ...
            // ????
            if (EXCEPTION_SUCCESS != init_hcc_head_and_send(st_hcc_transfer_param, pst_exception_data,
