...
static inline int32 ps_change_baud_rate_retry(int64 baud_rate, uint8 flowctl_status)
{
    ...
    while (0 != ps_change_uart_baud_rate(baud_rate, flowctl_status)) {
    ...
    if ((ps_core_d->tty) && (ps_core_d->tty->port)) {
        ...
        if (UART_PCLK_FROM_SENSORHUB == get_uart_pclk_source()) {
            ...
            if (1 == getSensorMcuMode()) {
                ...
                while ((ST_WAKEUP != get_iomcu_power_state()) && (ST_POWERON_OTHER != get_iomcu_power_state()) &&
                       (ST_POWERON != get_iomcu_power_state())) {
...
int32 bfgx_dev_power_on(void)
{
    ...
    if (!ir_only_mode) {
        if ((BOARD_VERSION_HI1103 == get_hi110x_subchip_type()) ||
            (BOARD_VERSION_HI1102A == get_hi110x_subchip_type())) {
            if (wlan_is_shutdown()) {
