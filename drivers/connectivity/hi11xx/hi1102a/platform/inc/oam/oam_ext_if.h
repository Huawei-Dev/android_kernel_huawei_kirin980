...
OAL_STATIC OAL_INLINE oal_switch_enum_uint8 oam_get_log_switch(oal_uint32 ul_para)
{
    ...
    /* ????????????????????????????????????????????info?????????????????????????????? */
    if (OAL_SWITCH_OFF == oam_mng_ctx.st_log_ctx.en_global_log_switch) {
        return OAL_SWITCH_OFF;
    } else if (OAL_SWITCH_OFF == oam_mng_ctx.st_log_ctx.st_vap_log_info.aen_vap_log_switch[uc_vap_id]) {
        return OAL_SWITCH_OFF;
