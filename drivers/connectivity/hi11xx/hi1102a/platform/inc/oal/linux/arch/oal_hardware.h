 ...
 * ????????  : E5????dbac????????????????,??????:drv_timer.h
 * ????????  : ul_id:??????ID??p_func:????????;l_arg:??;ul_timer_value:??????;
 *             ul_mode:??;ul_type:????;(0:????ms??1:????us;)
 */
#if (_PRE_CONFIG_TARGET_PRODUCT == _PRE_TARGET_PRODUCT_TYPE_E5)
...
/* ??var??[pos, pos + bits-1]????????,  pos??0???????? e.g oal_clearl_bits(var, 4, 2) ??????Bit5~4???? */
...
/* ??var??[pos, pos + bits-1]??????????val,  pos??0???????? e.g oal_setl_bits(var, 4, 2, 2) ??????Bit5~4??????b'10 */
