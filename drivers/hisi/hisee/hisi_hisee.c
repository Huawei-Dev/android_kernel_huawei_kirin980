...
static hisee_driver_function g_hisee_atf_function_list[] = {
	...
	{"inse_shutdown_swipe_t_pmu_en", inse_shutdown_swipe_t_nfc_en_func}, /* ????????????????PMU????NFC???????? */
	{"inse_shutdown_swipe_t_pmu_dis", inse_shutdown_swipe_t_nfc_dis_func}, /* ????????????????PMU????NFC???????? */

	{"CASD", hisi_receive_casd_func},
	{"VERIFYCASD", hisi_verify_casd_func},
	{"hisee_factory_check", hisee_factory_check_func},
	{"nfc_irq_switch", HISEE_DRIVER_FUN(hisee_nfc_irq_switch_func)},
...
static void show_hisee_module_status(char *buff)
{
	...
	ret = snprintf_s(counter_value, sizeof(counter_value), sizeof(counter_value), "%d\n", g_runtime_cosid);
	if (ret == HISEE_SECLIB_ERROR) {
