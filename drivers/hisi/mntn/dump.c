...
static int __init early_parse_memdumpaddr_cmdline(char *p)
{
	...
	if (EOK != memcpy_s(memdump_addr, MEMDUMP_ADDR_LEN, p, MEMDUMP_ADDR_LEN)) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
	}
	memdump_addr[MEMDUMP_ADDR_LEN] = 0;
	memcpy(memdump_end, p + MEMDUMP_ADDR_LEN+1, MEMDUMP_END_LEN);
	memdump_end[MEMDUMP_END_LEN] = 0;
	memcpy(memdump_size, p + MEMDUMP_ADDR_LEN+MEMDUMP_END_LEN+2, MEMDUMP_SIZE_LEN);
