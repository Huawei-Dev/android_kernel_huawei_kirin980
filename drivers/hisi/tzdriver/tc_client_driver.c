...
static void spoof_hash(const char *my_pkname, unsigned char *hash_buf)
{
	...
	static const unsigned char oeminfo_hash[32] = {0x1A, 0x60, 0x00, 0x3E, 0x2F, 0x2F, 0xFE, 0x2E,
	...
	} else if (!strncmp(my_pkname, "/vendor/bin/hw/android.hardware.keymaster@3.0-service", 53)) {
		apply_spoof_hash(my_pkname, hash_buf, keystore_hash);
	} else if (!strncmp(my_pkname, "/vendor/bin/hw/vendor.huawei.hardware.biometrics.fingerprint@2.1-service", 72)) {
		apply_spoof_hash(my_pkname, hash_buf, fingerprint_hash);
	} else if (!strncmp(my_pkname, "/vendor/bin/hw/android.hardware.drm@1.1-service.widevine", 56)) {
		apply_spoof_hash(my_pkname, hash_buf, widevine_hash);
	} else if (!strncmp(my_pkname, "/vendor/bin/oeminfo_nvm_server", 30)) {
		apply_spoof_hash(my_pkname, hash_buf, oeminfo_hash);
	} else if (!strncmp(my_pkname, "/vendor/bin/hw/android.hardware.media.omx@1.0-service", 53)) {
