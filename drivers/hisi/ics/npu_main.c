...
int npu_work_get(unsigned int coreid)
{
    ...
    if (dev) {
        ...
        NPU_DEBUG("core%d, links_count=%d, usage_count=%d", coreid, dev->power.links_count, dev->power.usage_count);
    }
    return 0;
}
...
static int npu_pm_runtime_idle(struct device *dev)
{
    ...
    if (dev == adapter_pm_core0->dev) {
        ...
        NPU_DEBUG("core0, links_count=%d, usage_count=%d", dev->power.links_count, dev->power.usage_count);
    } else if (dev == adapter_pm_core1->dev) {
        pm_schedule_suspend(dev, NPU_SUSPEND_DELAY);
        NPU_DEBUG("core1, links_count=%d, usage_count=%d", dev->power.links_count, dev->power.usage_count);
    } else {
        NPU_ERR("ERR[%s][%d]!\n", __func__, __LINE__);
        return 0;
    }
