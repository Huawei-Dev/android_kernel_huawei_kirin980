...
retry:
		...
		if (!(mci_readl(host, INTMASK) & (unsigned int)SDMMC_INT_SDIO((unsigned int)slot->id))
...
static int dw_mci_get_ro(struct mmc_host *mmc)
{
	...
	if (gpio_ro >= 0)
		...
		read_only =
			mci_readl(slot->host, WRTPRT) & ((unsigned int)1 << id) ? 1 : 0;
...
int dw_mci_get_cd(struct mmc_host *mmc)
{
	...
	if (brd->quirks & DW_MCI_QUIRK_BROKEN_CARD_DETECTION)
		...
		present = (mci_readl(slot->host, CDETECT) & ((unsigned int)1 << id))
			== 0 ? 1 : 0;
	if (present)
...
static void dw_mci_enable_sdio_irq(struct mmc_host *mmc, int enb)
{
	...
	if (enb) {
		...
		mci_writel(host, INTMASK,
			   (int_mask | (unsigned int)SDMMC_INT_SDIO((unsigned int)slot->id)));
	} else {
		mci_writel(host, INTMASK,
			   (int_mask & ~(unsigned int)SDMMC_INT_SDIO((unsigned int)slot->id)));
	}
	...
	if (pending) {
		...
		/*
                ...
                if (host->quirks & DW_MCI_QUIRK_IDMAC_DTO) {
                        if (!pending &&
                            ((mci_readl(host, STATUS) >> 17) & (unsigned int)0x1fff))
                                pending |= SDMMC_INT_DATA_OVER;
                }

		if (pending & SDMMC_INT_CMD_DONE) {
			u32 cmd = mci_readl(host, CMD) & (unsigned int)0x3f;
			if (cmd == SD_SWITCH_VOLTAGE &&
				!(mci_readl(host, STATUS) & (unsigned int)SDMMC_STATUS_BUSY)) {
