...
int send_mag_calibrate_data_to_mcu(void)
{
	...
	if (g_mag_folder_function == 1) {
		memcpy(msensor_folder_calibrate_data, user_info.nv_data, MAG_FOLDER_CALIBRATE_DATA_NV_SIZE);
		hwlog_info("send mag_sensor data %d, %d, %d to mcu success\n", msensor_folder_calibrate_data[0],
			   msensor_folder_calibrate_data[1], msensor_folder_calibrate_data[2]);
		if (send_calibrate_data_to_mcu(TAG_MAG, SUB_CMD_SET_OFFSET_REQ,
		msensor_folder_calibrate_data, MAG_FOLDER_CALIBRATE_DATA_NV_SIZE, false))
			return -1;
	} else if (akm_cal_algo == 1) {
...
int write_magsensor_calibrate_data_to_nv(char *src)
{
	...
	if (g_mag_folder_function == 1) {
		mag_size = MAG_FOLDER_CALIBRATE_DATA_NV_SIZE;
		memcpy(&msensor_folder_calibrate_data, src, sizeof(msensor_folder_calibrate_data));
...
void reset_calibrate_data(void)
{
	...
	if (g_mag_folder_function == 1) {
		...
		msensor_folder_calibrate_data, MAG_FOLDER_CALIBRATE_DATA_NV_SIZE, true);
	} else if (akm_cal_algo == 1) {
...
void set_als_extend_prameters(als_para_normal_table *als_para_diff_tp_color_table, int arraysize)
{
	...
	memcpy(als_data.als_extend_data,
		(als_para_diff_tp_color_table + table_index)->als_para,
		sizeof(s16) * (als_para_diff_tp_color_table + table_index)->len >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE
		? SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE
		: sizeof(s16) * (als_para_diff_tp_color_table + table_index)->len);
	min_threshold_num = (als_para_diff_tp_color_table + table_index)->len - 1;
	max_threshold_num = (als_para_diff_tp_color_table + table_index)->len - 2;
	minThreshold_als_para = (als_para_diff_tp_color_table + table_index)->als_para[min_threshold_num];
	maxThreshold_als_para = (als_para_diff_tp_color_table + table_index)->als_para[max_threshold_num];
}
...
void select_als_para(struct device_node *dn)
{
	...
	if (rohm_rgb_flag == 1) {
		...
		set_als_extend_prameters(&veml32185_als_para_diff_tp_color_table[0],
				ARRAY_SIZE(veml32185_als_para_diff_tp_color_table));
	} else {
