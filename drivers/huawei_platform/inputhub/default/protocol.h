...
typedef struct interval_param{
	...
	//3????????????????AP??????????????????????
	uint8_t	mode;
	uint8_t	reserved[3]; //reserved[0]????motion????????????
...
typedef struct {
	pkt_header_t hd;
	//1??????sensorlist??0????filelist??2????????????
	uint8_t file_flg;
	//??????????????????
	//(MAX_PKT_LENGTH-sizeof(PKT_HEADER)-sizeof(End))/sizeof(UINT16)
	uint8_t file_count;//file??????sensor??????
	uint16_t file_list[];//fileid????sensor??tag
...
enum {
	...
	FILE_VD6281_ALS,// 140
	FILE_EGIS_UD_FP, // 141
	FILE_BMI260_ACC = 156,                        /* 156 */
	FILE_BMI260_GYRO = 157,                       /* 157 */
