...
static bool g_ccalg_start = false;
int8_t g_ccalg_uid_cnt = 0;
 ...
 */
void emcom_xengine_active_ccalg(uint8_t *pdata, uint16_t len)
{
	uid_t uid;
	uint32_t alg;
	int8_t index;
	struct emcom_xengine_ccalg_config *pst_ccalg_config;

	/* input param check */
	if ((pdata == NULL) || (len == 0) || (len > IFNAMSIZ)) {
		EMCOM_LOGE("CCAlg interface name or length %d is error", len);
		return;
	}
	g_ccalg_start = true;
	pst_ccalg_config = (struct emcom_xengine_ccalg_config_data*)pdata;
	uid = pst_ccalg_config->uid;
	alg = pst_ccalg_config->alg;
	...
	if (index != INDEX_INVALID) {
		if (g_ccalg_uids[index].alg == alg) {
			...
			EMCOM_LOGD("CCAlg function is ready to change alg, uid: %u, alg from %u to %u", uid, g_ccalg_uids[index].alg, alg);
 ...
 */
void emcom_xengine_deactive_ccalg(uint8_t *pdata, uint16_t len)
{
	uid_t uid;
	int8_t index;
	if ((pdata == NULL) || (len == 0) || (len > IFNAMSIZ)) {
...
void Emcom_Xengine_EvtProc(int32_t event, uint8_t *pdata, uint16_t len)
{
	switch(event)
	{
		...
		case NETLINK_EMCOM_DK_STOP_FAST_SYN:
			...
			break;
#ifdef CONFIG_HUAWEI_OPMP
		case NETLINK_EMCOM_DK_OPMP_INIT_HEARTBEAT: // fall through
		case NETLINK_EMCOM_DK_OPMP_DEINIT_HEARTBEAT: // fall through
		case NETLINK_EMCOM_DK_OPMP_TRIGGER_HEARTBEAT:
			EMCOM_LOGD("emcom netlink received opmp heartbeat msg %d", event);
			opmp_event_process(event, pdata, len);
			break;
#endif
		case NETLINK_EMCOM_DK_ACTIVE_CCALG:
			EMCOM_LOGD(" emcom netlink active congestion control algorithm");
			emcom_xengine_active_ccalg(pdata, len);
			break;
		case NETLINK_EMCOM_DK_DEACTIVE_CCALG:
			EMCOM_LOGD(" emcom netlink deactive congestion control algorithm");
			emcom_xengine_deactive_ccalg(pdata, len);
			break;
		default:
