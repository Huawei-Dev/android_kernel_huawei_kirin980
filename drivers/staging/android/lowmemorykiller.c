...
#if defined CONFIG_LOG_JANK
#define JANK_BUFFER_SIZE 70
#define JANK_BUFFER_MAX_SIZE 256
#define JLID_PROCESS_KILL 396
static ulong lowmem_kill_count;
static ulong lowmem_free_mem;
static char g_jank_buffer[JANK_BUFFER_MAX_SIZE] = {0};
static int g_buf_pos = 0;
static DEFINE_SPINLOCK(g_buffer_lock);
#endif
...
static void upload_to_jank(struct task_struct *tsk, short oom_adj)
{
	...
	if (tsk->mm) {
		...
		snprintf(jbuffer, JANK_BUFFER_SIZE, "%8d, %16s, %8luKb, %8luKb, %8luKb, %4hd;",
			tsk->pid, tsk->comm, anon << (PAGE_SHIFT-10),
			file << (PAGE_SHIFT-10), shmem << (PAGE_SHIFT-10), oom_adj);
		if (spin_trylock(&g_buffer_lock)) {
			if (g_buf_pos + JANK_BUFFER_SIZE >= JANK_BUFFER_MAX_SIZE) {
				LOG_JANK_D(JLID_PROCESS_KILL, "%s", g_jank_buffer);
				g_buf_pos = 0;
			}
			snprintf(&(g_jank_buffer[g_buf_pos]), JANK_BUFFER_SIZE, "%s", jbuffer);
			g_buf_pos += strlen(jbuffer);
