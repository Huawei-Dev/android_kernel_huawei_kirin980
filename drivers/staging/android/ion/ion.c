			     ...
			     unsigned int flags, bool grab_handle)
{
	...
	if (heap_id_mask & 0x1 << ION_DRM_HEAP_ID)
		heap_id_mask |= 0x1 << ION_DRM_CPA_HEAP_ID;
#endif
...
{
	struct ion_buffer *buffer = vma->vm_private_data;
	struct ion_vma_list *vma_list = NULL;
			      ...
			      struct file *f, unsigned int  fd)
{
	...
	if (is_dma_buf_file(f)) {
		...
		if (dbuf && (dbuf->owner == THIS_MODULE) && dbuf->priv) {
			ibuf = dbuf->priv;

			*(args->total_ion_size) += dbuf->size;
			seq_printf(args->seq, "%16s %16u %16u %16zu %16llu %16u %16s\n",
				tsk->comm, tsk->pid,
...
#ifdef CONFIG_ION_HISI_CPA
	if (ret) {
		pr_info("%s: phys turn to cpa_heap!\n", __func__);
		ret = ion_cpa_heap_phys(buffer->heap, buffer, addr, len);
	}
#endif
	mutex_unlock(&client->lock);
