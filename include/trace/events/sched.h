...
TRACE_EVENT(sched_cpu_util,
	...
	TP_PROTO(int cpu, long wake_util, u64 irqload, int high_irqload),

	TP_ARGS(cpu, wake_util, irqload, high_irqload),

	TP_STRUCT__entry(
		__field(unsigned int, cpu)
		__field(unsigned int, nr_running)
		__field(long, cpu_util)
		__field(long, cpu_util_wake)
	...
	TP_printk("cpu=%d nr_running=%d cpu_util=%ld cpu_util_wake=%ld"
		  " cpu_util_cum=%ld capacity_curr=%u capacity=%u"
		  ...
		  __entry->cpu_util_wake, __entry->cpu_util_cum,
		  __entry->capacity_curr, __entry->capacity,
		  __entry->capacity_orig, __entry->capacity_min,
		  __entry->idle_state, __entry->irqload, __entry->online,
		  __entry->isolated, __entry->reserved, __entry->high_irqload)
);
...
TRACE_EVENT(sched_load_balance_nohz_kick,
	...
	TP_fast_assign(
		...
		__entry->misfit_task_load	= cpu_rq(cpu)->misfit_task;
