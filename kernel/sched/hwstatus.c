...
void sched_hwstatus_iodelay_caller(struct task_struct *tsk, u64 delta)
{
	...
	if((tsk) && (tsk->static_vip)) {
		...
		snprintf_s(caller.caller_iodelay, CALLER_NAME_LEN, CALLER_NAME_LEN-1, "[%d,%d]%s W:%d P:[%d,%d] N:%s T:%Lu %pS\n",
	...
	for(i=0;  i<FGTASK_MAX; i++){
		...
		if(version > VERSION_V1) {
			...
			if(caller.ktime_iodelay >= ktime_last) {
				...
				strncpy_s(hwstatus_rst.caller.caller_iodelay, CALLER_NAME_LEN, caller.caller_iodelay, CALLER_NAME_LEN-1);
				if(version > VERSION_V2) {
					strncpy_s(hwstatus_rst.stack.stack, STACK_DUMP_SIZE, stack_dump.stack, STACK_DUMP_SIZE-1);
				}
			} else {
