...
#define SCHED_SHORT_SLEEP	(1ULL * NSEC_PER_MSEC)
#define SCHED_SLEEP_ACCOUNT_MAX	(8ULL * NSEC_PER_MSEC)
#define SCHED_SLEEP_DECAY	(8)

bool walt_task_sleep_short(struct task_struct *p)
{
#ifdef CONFIG_SCHED_HISI_USE_WALT
	return p->ravg.avg_sleep_time < SCHED_SHORT_SLEEP;
#else
	/*
	 * Do not consider task_sleep_short case when use PELT.
	 * (task_sleep_short will bypass active load balance in
	 * idle_balance)
	 */
	return false;
#endif
...
void walt_update_task_avg_sleep(struct task_struct *p, u64 sleep_time)
{
	s64 diff = min(sleep_time, SCHED_SLEEP_ACCOUNT_MAX) - p->ravg.avg_sleep_time;
	p->ravg.avg_sleep_time += diff / SCHED_SLEEP_DECAY;
}
	     ...
	     int event, u64 wallclock, u64 irqtime)
{
	...
	rcu_read_unlock();
	if (grp && grp->mode.freq_enabled &&
		rq->cluster == grp->preferred_cluster) {
...
void walt_init_new_task_load(struct task_struct *p)
{
	...
	p->ravg.avg_sleep_time = SCHED_SHORT_SLEEP;
